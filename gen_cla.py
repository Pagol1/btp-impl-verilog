...
def gen_cla():
    ...
    for i in range(num_stage):
        ST_ADDR.append("{}:0".format((BIT_LEN >> i)-1))
        ST_WIDTH.append(str(BIT_LEN>>i))
...
def gen_cla_pipe():
    ...
    gen_l = []
    prop_l = []
    op += pp("reg [" + ADDR + "] in_a_val, in_b_val;")
    op += pp("reg in_carry_val;")
    for i in range(len(ST_ADDR)):
        gen_l.append("gen" + f"{i:0{num_pad}d}")
        prop_l.append("prop" + f"{i:0{num_pad}d}")
        op += pp("wire [" + ST_ADDR[i] + "] " + gen_l[-1] + ", " + prop_l[-1] + ";")
        ## Pipeline
        op += pp("reg [" + ST_ADDR[i] + "] " + gen_l[-1] + "_val, " + prop_l[-1] + "_val;")
    i_ = num_stage
    gen_l.append("gen" + f"{i_:0{num_pad}d}")
    prop_l.append("prop" + f"{i_:0{num_pad}d}")
    op += pp("wire " + gen_l[-1] + ", " + prop_l[-1] + ";")
    op += pp("reg " + gen_l[-1] + "_val, " + prop_l[-1] + "_val;")
    ...
    i_ = num_stage
    op += pp("cla_gen_prop L" + f"{i_:0{num_pad}d}" + "(" + gen_l[i_-1] + "[1], " + gen_l[i_-1] + "[0], " + prop_l[i_-1] + "[1], " + prop_l[i_-1] + "[0], " + gen_l[i_] + ", " + prop_l[i_] + ");")
    ST_WIDTH.append(str(1))
    ## Pipeline
    op += pp("always @(posedge clk) begin")
    op += pp("in_a_val <= in_a & {" + WIDTH + "{rstn}};")
    op += pp("in_b_val <= in_b & {" + WIDTH + "{rstn}};")
    op += pp("in_carry_val <= in_carry & rstn;")
    for i in range(len(gen_l)):
        op += pp("{}_val <= {} & ".format(gen_l[i], gen_l[i]) + "{" + ST_WIDTH[i] + "{rstn}};")
        op += pp("{}_val <= {} & ".format(prop_l[i], prop_l[i]) + "{" + ST_WIDTH[i] + "{rstn}};")
    op += pp("end")
    ...
    lvl = 0
    op += pp("// Lvl{} //".format(lvl))
    op += pp("assign cry[0] = in_carry_val;")
    carry_mask[0] = 1
    for exp in range(num_stage-1):
        idx = 1 << exp
        op += pp("assign cry[{}] = {}_val[0] | ({}_val[0] & in_carry_val);".format(idx, gen_l[exp+1], prop_l[exp+1]))
        carry_mask[idx] = 1
    ...
    while not all_marked(carry_mask):
        ...
        for i in range(len(carry_mask)-1):
            for exp in range(num_stage-2):
                ...
                if (i + j) < len(carry_mask):
                    if carry_mask[i] == 1 and carry_mask[i+j] == 0:
                        op += pp("assign cry[{}] = {}_val[{}] | ({}_val[{}] & cry[{}]);".format(i+j, gen_l[exp+1], i>>exp, prop_l[exp+1], i>>exp, i))
                        carry_mask[i+j] = 1
    ...
    op += pp("generate")
    op += pp("for (j=0; j<{}/2; j=j+1) begin : RCA_CHAIN".format(WIDTH))
    op += pp("rca RCA(in_a_val[2*j+1:2*j], in_b_val[2*j+1:2*j], cry[j], out_s[2*j+1:2*j]);")
    op += pp("end")
    op += pp("endgenerate")

    op += pp("assign out_overflow = ({}_val | ({}_val & in_carry_val)) ^ ({}_val[{}] | ({}_val[{}] & cry[{}]));".format(
            gen_l[-1], prop_l[-1], gen_l[0], BIT_LEN-2, prop_l[0], BIT_LEN-2, len(carry_mask)-1
        ))
...
if __name__ == "__main__":
    ...
    # IO_PRE = "`BIT8_"
    # INT_PRE = "`CLA_8BIT_"
    # MODULE_FILE_NAME = "cla_8bit.v"
    # # ## FXP64_pipe
    # MODULE_FILE_NAME = "cla_8bit_pipe.v"
    # BIT_LEN = 8
    ##
    INDENT = 0
    op = gen_cla_pipe()
